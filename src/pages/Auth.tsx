...
import { createNest, joinNest } from '@/lib/firestore';

const Auth = () => {
  const navigate = useNavigate();
  const { setAuth, setUserProfilePic, isAuthenticated } = useApp();
  ...
  useEffect(() => {
    if (currentUser) {
      if (!name) setName(currentUser.displayName?.split(' ')[0] || '');
      if (!profilePic && currentUser.photoURL) setProfilePic(currentUser.photoURL);
    }
  }, [currentUser]); // Removed problematic dependencies
  ...
  const handleCreate = async () => {
    ...
    setErrorMsg('');
    try {
      const code = await createNest(currentUser, name, avatar, profilePic);
      setAuth(name, avatar, code);
      if (profilePic) setUserProfilePic(profilePic);
    } catch (e: any) {
      setErrorMsg(e.message || 'Failed to create nest');
    } finally {
  ...
  const handleJoin = async () => {
    if (!name.trim() || joinCode.length < 6 || !currentUser) return;
    setIsProcessing(true);
    setErrorMsg('');
    try {
      await joinNest(joinCode, currentUser, name, avatar, profilePic);
      setAuth(name, avatar, joinCode);
      if (profilePic) setUserProfilePic(profilePic);
    } catch (e: any) {
      setErrorMsg(e.message || 'Invalid Love Code or Nest is full');
    } finally {
      setIsProcessing(false);
    }
  };
        ...
        {!currentUser ? (
          <div className="space-y-6">
            <p className="text-center text-sm text-muted-foreground mb-8">
              Sign in securely to sync your universe with your partner in real-time.
            </p>
            <button 
              onClick={() => signInWithGoogle().catch(e => setErrorMsg(e.message))}
              className="w-full rounded-2xl border border-border/30 bg-white dark:bg-muted/10 py-4 text-sm font-bold text-foreground flex items-center justify-center gap-4 shadow-sm hover:shadow-md hover:bg-muted/5 transition-all"
            >
          ...
          <>
            <div className="flex justify-between items-center mb-6">
              <span className="text-xs text-muted-foreground">Signed in as {currentUser.email}</span>
              <button onClick={() => signOut()} className="text-xs text-muted-foreground flex items-center gap-1 hover:text-foreground">
                <LogOut className="w-3 h-3" /> Sign Out
              </button>
            </div>

            <div className="relative flex p-1.5 bg-muted/30 rounded-2xl mb-8 backdrop-blur-sm border border-border/10">
              <motion.div
              ...
              >
                        ...
                        {avatarEmojis.map((emoji, i) => (
                          <motion.button
                            ...
                            className={cn(
                              ...
                              avatar === i && !profilePic ? 'bg-primary/10 border-primary shadow-primary/10 ring-1 ring-primary/40 scale-105' : 'bg-muted/30 border-transparent'
                            )}
                            whileTap={{ scale: 0.9 }}
                ...
                <div className="pt-2">
                  <motion.button
                    ...
                    className={cn("w-full rounded-2xl bg-primary py-4 text-base font-bold text-primary-foreground shadow-xl shadow-primary/20 hover:shadow-primary/30 transition-all flex items-center justify-center gap-3",
                      isProcessing && "opacity-70 cursor-not-allowed")}
                    whileTap={!isProcessing ? { scale: 0.98 } : {}}
                    onClick={tab === 'create' ? handleCreate : handleJoin}
                  >
                    {isProcessing ? 'Connecting Server...' : tab === 'create' ? 'Launch Your Nest 🚀' : 'Join the Nest 🥂'}
                  </motion.button>
                </div>
              </motion.div>
            </AnimatePresence>
